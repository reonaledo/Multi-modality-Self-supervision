...
# from pytorch_pretrained_bert.modeling import BertModel, AutoModel
...
class MultimodalBertEncoder(nn.Module):
    def __init__(self, args):
        ...
        if self.args.num_image_embeds >0:
    ...
    def forward(self, input_txt, attention_mask, segment, input_img):
        ...
        if self.args.num_image_embeds >0:
            attention_mask = torch.cat(
                [torch.ones(bsz, self.args.num_image_embeds + 2).long().cuda(),attention_mask],dim=1,)
