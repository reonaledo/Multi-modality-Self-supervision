...
class Img2txtDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_set, file_src, image_root, split, batch_size, tokenizer, max_len, file_valid_jpgs ='tmp.json', use_num_imgs=-1, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[], s2s_prob=0, bi_prob=1, tasks='img2txt'):
        super().__init__()
        ...
        if tasks == 'img2txt':
            ...
            for idx, src in enumerate(tqdm(img_dat)): # load each img path & txt
                ...
                self.ex_list.append((src_tk, tokenizer.tokenize(tgt_tk), tgt_label, 1, {'answers': ['dummy']}))                        

                counter += 1

        elif tasks == 'vqa2': # just in case
            counter = 0
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, block_mask=False, new_segment_ids=False, truncate_config={}, mask_image_regions=False, mode=None, len_vis_input=None, vis_mask_prob=0, enable_butd=False, region_bbox_file='', region_det_file_prefix='', local_rank=-1, load_vqa_ann=False):
        super().__init__()
        ...
        if not enable_butd:
            ...
            if load_vqa_ann: pass

    def __call__(self, instance):
        
        img_path, tokens_b, random_label, random_itm_label = instance[:4]
        # mlm_dataset = img_path, tokens_b
        # itm_dataset = img_path, tokens_b, paired_itm_label, ran_sampled_txt,random_label, random_itm_label
        tokens_a = ['[UNK]'] * self.len_vis_input

        # itm dataset
        itm_label = torch.tensor(random_itm_label)

        # 학습할때 txt를 너무 많이 truncate함. 20token만 남겨둠. 기준은 randomly 앞에서 부터 또는 뒤에서 부터 truncate를 함.
        ...
        if not self.enable_butd:
            ...
            # vis_pe = torch.arange(0,2048,1, dtype=torch.int)
            vis_pe = torch.arange(2048, dtype=torch.float)
            vis_pe = vis_pe.unsqueeze(0).expand(len(tokens_a), 2048)
            ans_tk = 0
        else: 
            pass
        return (input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, itm_label,self.task_idx, img, vis_masked_pos, vis_pe, ans_tk)
