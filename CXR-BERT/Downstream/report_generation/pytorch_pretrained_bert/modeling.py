...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.img_embeddings = nn.Linear(2048, config.hidden_size)
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, position_ids=None, vis_input=True, len_vis_input=None):
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, len_vis_input=None): #len_vis_input=49):
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, enable_butd=False, len_vis_input=None, tasks='img2txt'):#=49, tasks='img2txt'):
        ...
        self.crit_itm_lm = nn.CrossEntropyLoss(reduction='none')
        # prediction_scores_itm, _ = self.cls(
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, ans_labels=None, next_sentence_label=None, masked_pos=None, masked_weights=None, task_idx=None, vis_masked_pos=[], mask_image_regions=False, drop_worst_ratio=0.2, vqa_inference=False):
        ...
        prediction_scores_masked, itm_output = self.cls(
            sequence_output_masked, pooled_output, task_idx=task_idx)
        ...
        itm_loss = self.crit_itm_lm(
            itm_output.view(-1, 2), next_sentence_label.view(-1))

        ######################   measuere   ######################
        # print("input_ids",input_ids) 
        itm_out = [element.item() for element in itm_output.argmax(dim=-1).flatten()]
        align_lt = [element.item() for element in next_sentence_label.flatten()]
        matched_itm = [index for index, (e1, e2) in enumerate(zip(itm_out, align_lt)) if e1 == e2]

        out_list = [element.item() for element in prediction_scores_masked.argmax(dim=-1).flatten()]
        label_list = [element.item() for element in masked_lm_labels.flatten()]
        masked_index2 = [itr for itr, val in enumerate(label_list) if val != 0] #input 중 masking된 친구들의 index값 
        ...
        matched = [index for index, (e1, e2) in enumerate(zip(aa, bb)) if e1 == e2]
        # masked_lm_acc = prediction_scores_masked.argmax(dim=-1).eq(masked_lm_labels).sum().item()
        ##################################################################

        itm_loss = torch.sum(itm_loss) + 1e-5
        # itm_loss = torch.sum(itm_loss.sum(-1)) + 1e-5
        model_loss = masked_lm_loss + itm_loss
        vis_pretext_loss = masked_lm_loss.new(1).fill_(0)

        mlm_acc = masked_lm_loss.new(1).fill_(round(len(matched)/len(masked_index2)* 100, 3))
        itm_acc = masked_lm_loss.new(1).fill_(round(len(matched_itm)/len(align_lt)* 100, 3))

        return  masked_lm_loss, itm_loss, vis_pretext_loss, masked_lm_loss.new(1).fill_(0), mlm_acc, itm_acc#, #, cumulated_lm_acc, cumulated_labelno
                
""" for VLP, based on UniLM """
class BertForSeq2SeqDecoder(PreTrainedBertModel):
# class BertForSeq2SeqDecoder(PreTrainedModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2,
                 ...
                 ngram_size=3, min_len=0, enable_butd=False, len_vis_input=None):#len_vis_input=49):
        super(BertForSeq2SeqDecoder, self).__init__(config)

        # config = AutoConfig.from_pretrained("google/bert_uncased_L-4_H-512_A-8")
        # print("config",config)
        # input("STOP!!!")
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, sample_mode='greedy'):
        ...
        # vision positional embedding을 위해 현재, VLP 코드에는 빠져 있는 부분을 채우는 중.
        # VLP에는 BUTD 방식으로 하기 때문에 이에 맞춰서 vis_pe를 좌표값을 넣고 그를 임베딩시켜줌
        # 우리 방식에선 vis_pe를 0으로 지정 또는 0~1000까지 range해 값을 임의로 넣고 matrix로 만들어주려해봄
        # 현재 문제는 butd 아닌 방식에 대한 vision position embedding이 원래 코드에는 빠져 있었고
        # 이를 채우기 위해 임의로 구성함. ViT에서 이미지 임베딩에서 포지션을 각기 다르게 배정해줬는데 이와 비슷한 방식으로 지금 시도해본거 
        # 
        # 문제는 vis pe를 굳이 안넣어도 괜찮지 않을까 생각해봤는데, 왜냐면 posion ids가 버트에서 비슷한 역할하지 않나 하는 생각에
        
        vis_pe = self.vis_pe_embed(vis_pe) # image region positional encodings


        if self.search_beam_size > 1:
            # print("LETS's do beam_search",self.search_beam_size)
        ...
        # 아래 while 문이 masking prediction 하는 부분. 
    ...
    def beam_search(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None):
        ...
        while next_pos < output_length:
            ...
            print("curr_token_type_ids",curr_token_type_ids.size())
            print("curr_position_ids",curr_position_ids.size())
            print("curr_attention_mask",curr_attention_mask.size())
