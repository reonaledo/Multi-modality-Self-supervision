...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, vis_feats=None, vis_pe=None,vis_input=False, len_vis_input=49):
...
class ImageBertEmbeddings(nn.Module):
    ...
    def forward(self, input_id, input_imgs, vis_pe, token_type_ids):  # img_embed_out = self.img_embeddings(img, img_tok)
        ...
        if self.args.img_postion:
            ...
            # print("token_position_embeddings.size()",token_position_embeddings.size())
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, args, num_labels=2, enable_butd=False, len_vis_input=49, tasks='img2txt'):
        ...
        self.cls = BertPreTrainingHeads(
            config, bert.embeddings.word_embeddings.weight, num_labels=num_labels) # num_labels not applicable for VLP
        self.apply(self.init_bert_weights)
    ...
    def forward(self, img, _, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, ans_labels=None, next_sentence_label=None, masked_pos=None, masked_weights=None, task_idx=None, vis_masked_pos=[], mask_image_regions=False, drop_worst_ratio=0.2, vqa_inference=False):
                    #img, vis_pe, input_ids, segment_ids, input_mask, lm_label_ids, ans_labels, is_next, masked_pos=masked_pos, masked_weights=masked_weights, task_idx=task_idx,
        ...
        if mask_image_regions:
            ...
            # txt_embed_out = self.txt_embeddings(input_ids[:, self.len_vis_input+2:], position_embeddings)  # txt_embed_out: torch.Size([32, 507, 768])
        ...
        if self.tasks == 'vqa2':
            ...
            return masked_lm_loss, vis_pretext_loss, masked_lm_loss.new(1).fill_(0), mlm_acc
...
class CXRBertDecoder(nn.Module):  # MultimodalBertEncoder, BERT
    def __init__(self, config, args):
        ...
        self.pooler = bert.pooler
        # self.cls = BertPreTrainingHeads(
        #     config, bert.embeddings.word_embeddings.weight, num_labels=2) # num_labels not applicable for VLP
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, args, mask_word_id=0, num_labels=2,
                 ...
                 ngram_size=3, min_len=0, enable_butd=False, len_vis_input=200):
    ...
    def forward(self, vis_feats, _, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, sample_mode='greedy'):

        if self.search_beam_size > 1:
            return self.beam_search(vis_feats, input_ids, token_type_ids, position_ids, attention_mask, task_idx)
        ...
        while next_pos < output_length:
            ...
            # print("curr_token_type_ids",curr_token_type_ids)
    ...
    def beam_search(self, vis_feats, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None):
