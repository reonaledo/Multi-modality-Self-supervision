...
def main(args):
    ...
    for s in studies:
        ...
        print("\n")
        print("\n")
        print("what is the name of study : ",s)
        print("\n")
        
        with open(reports_path / s, 'r') as fp:
            text = ''.join(fp.readlines())

        # get study string name without the txt extension        
        s_stem = s[0:-4] #load sxxxxxx without .txt

        # custom rules for some poorly formatted reports
        # input()
        # print("custom_indices",custom_indices)
        # print("custom_section_names",custom_section_names)
        # input()
        if s_stem in custom_indices:
            idx = custom_indices[s_stem]
            patient_studies.append([s_stem, text[idx[0]:idx[1]]])
            input("custom_indices")
        ...
        if s_stem in custom_section_names:
            ...
            print("len(sn)",len(sn))
            print("len(sn)",sn)
            print("type(sn)",type(sn))

            if isinstance(sn, list):
                bundle = []
                for itr in sn:
                    idx = list_rindex(section_names, itr)
                    bundle.extend([sections[idx].strip()])
                print("bundle",''.join(bundle))
                # print("bundle[0]",bundle[0])
                patient_studies.append([s_stem,''.join(bundle)])
            else:
                idx = list_rindex(section_names, sn)
                patient_studies.append([s_stem, sections[idx].strip()])
            input("custom_section_names")
            print("patient_studies",patient_studies)
            continue
        ...
        if idx == -1:
            ...
            print(f'no impression/findings: {reports_path / s}')
        else:
    ...
    if len(patient_studies) > 0:
        ...
        #     print("row",row)
        #     input("Lets make txt files!!!!!!")

        #     with open(output_path / str(row[0])+".txt", "w") as fp:
        #         fp.write(''.join(row(text=True)))
        #         input("Lets make txt files!!!!!!")
