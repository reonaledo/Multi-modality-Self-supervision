...
class CXRBertEncoder(BertPreTrainedModel):
    ...
    def forward(self, cls_tok, input_txt, attn_mask, segment, input_img, sep_tok):
        ...
        if self.args.disturbing_mask:
            ...
            encoder_input = torch.cat([cls_out, img_embed_out, sep_out, txt_cls_out, txt_embed_out],
                                      1)  # TODO: Check B x (TXT + IMG) x HID

            encoded_layers = self.encoder(
                encoder_input, extended_attn_mask, output_hidden_states=False)

            cls_represent = encoded_layers[-1][:, 0] * encoded_layers[-1][:,
                                                       self.args.num_image_embeds + 2]  # element wisely multiplication.
            ...
            encoded_layers = self.encoder(
                encoder_input, extended_attn_mask, output_hidden_states=False, output_attentions=True,
            )
