...
def get_args(parser):
    ...
    parser.add_argument("--savedir", type=str, default="/home/hglee/cxr-bert/classification/output/")
    ...
    if scenari == 1:
        ...
        parser.add_argument("--name", type=str, default="scenario_1")
    ...
    # # scenari 2> Partially Auto-regressive (img36, txt128)_bert_small
        ...
        parser.add_argument("--name", type=str, default="scenario_2")

    # # scenari 3> Mixed Attention (img36, txt128)_bert_small
    elif scenari == 3:
        parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_s2s")
        parser.add_argument("--name", type=str, default="scenario_3")

    # # scenari 4> seq2seq (img36, txt128)_bert_small
    elif scenari == 4:
        parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_random_50prob")
        parser.add_argument("--name", type=str, default="scenario_4")

    ############################################################################################################

    # # # scenari 5> h&h
    # parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_full_attn")
    # parser.add_argument("--name", type=str, default="scenario_5")

    # # # scenari 6> h&h
    # parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_full_attn")
    # parser.add_argument("--name", type=str, default="scenario_6")

    # # # scenari 7> h&h
    # parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_full_attn")
    # parser.add_argument("--name", type=str, default="scenario_7")

    # # # scenari 8> h&h
    # parser.add_argument("--loaddir", type=str, default="/home/mimic-cxr/model/12.31_img36_txt128_full_attn")
    # parser.add_argument("--name", type=str, default="scenario_8")
...
def auc_graph():
    ...
    colors = ['aqua', 'darkorange', 'cornflowerblue', 'teal', 'indigo', 'magenta', 'lightblue', 'gold', 'tan',
              'slategrey', 'gray', 'coral', 'peru', 'olivedrab', 'pink']
...
def model_eval(i_epoch, data, model, args, criterion, device, store_preds=False):
    with torch.no_grad():
        ...
        for batch in data:
            ...
            # print('tgt:', tgt.size())
            """
            out: torch.Size([32, 15])
            tgt: torch.Size([32, 15])
            """
            if args.task_type == "multilabel":
...
def model_forward(i_epoch, model, args, criterion, batch, device):
    ...
    if args.model == "bow":
        ...
        mask, segment = mask.to(device), segment.to(device)
        out = model(txt, mask, segment, img)
    # print('out.size:', out.size())  # torch.Size([32, 15])
    # print('tgt.size:', tgt.size())  # torch.Size([32, 15])
